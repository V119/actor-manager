  ...
  <div class="flex-1 overflow-y-auto bg-background p-8 pt-24 min-h-screen text-on-surface">
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <section class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <article class="rounded-2xl border border-amber-300/20 bg-amber-500/10 p-4">
          <p class="text-xs tracking-[0.16em] uppercase text-amber-100/70">待处理单数</p>
          <p class="mt-2 text-2xl font-bold text-amber-100">{{ pendingCount }}</p>
        </article>
        <article class="rounded-2xl border border-emerald-300/20 bg-emerald-500/10 p-4">
          <p class="text-xs tracking-[0.16em] uppercase text-emerald-100/70">已成功单数</p>
          <p class="mt-2 text-2xl font-bold text-emerald-100">{{ succeededCount }}</p>
        </article>
      ...
      <section class="rounded-2xl border border-sky-400/10 bg-surface/50 p-5">
        <div class="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          <div class="flex items-center gap-3">
            <label class="text-xs text-on-surface-variant">状态筛选</label>
            <select
              v-model="statusFilter"
              class="rounded-xl border border-sky-300/20 bg-slate-950/30 px-3 py-2 text-sm text-on-surface outline-none transition focus:border-sky-300/40 focus:ring-1 focus:ring-sky-300/30"
            >
        ...
        <div v-else-if="!filteredWithdrawals.length" class="mt-4 rounded-xl border border-sky-300/10 bg-slate-950/20 px-4 py-6 text-sm text-on-surface-variant">
          当前没有匹配条件的提现记录。
        </div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
              <div>
                <div class="flex flex-wrap items-center gap-2">
                  <p class="text-sm font-semibold text-sky-50">{{ item.actor_name || `演员#${item.actor_id}` }}</p>
              ...
              <div v-if="isReviewable(item.status)" class="flex flex-wrap gap-2">
                <button
                  type="button"
                  class="rounded-full border border-emerald-300/30 bg-emerald-500/10 px-3 py-1.5 text-xs text-emerald-100 transition hover:bg-emerald-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                ...
                <button
                  type="button"
                  class="rounded-full border border-amber-300/30 bg-amber-500/10 px-3 py-1.5 text-xs text-amber-100 transition hover:bg-amber-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                  :disabled="reviewingNo === item.out_withdraw_no"
