...
const template = ref({
  version: 1,
  source_document_name: 'AI肖像权独家授权合作协议.docx',
  is_ready: false
})
...
async function loadTemplate() {
  ...
  resetMessages()
  try {
    const payload = await apiRequest('/admin/agreement/template', { token: authStore.state.token })
    applyTemplate(payload)
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '协议模板加载失败'
  }
}
...
async function saveTemplate() {
  ...
  resetMessages()
  try {
    const payload = await apiRequest('/admin/agreement/template', {
      method: 'PUT',
