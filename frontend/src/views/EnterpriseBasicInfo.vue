    ...
    <main class="max-w-5xl mx-auto px-6 md:px-8 pt-24 pb-14 space-y-8">
      <header class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <div class="text-xs text-on-surface-variant">
          资料完整度：<span class="font-semibold text-sky-300">{{ completionPercentage }}%</span>
        </div>
      </header>
      ...
      <section class="rounded-2xl border border-sky-400/10 bg-surface/65 p-5 md:p-6 backdrop-blur-xl">
        <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div class="flex items-center gap-4">
            <div class="flex h-16 w-16 items-center justify-center rounded-2xl border border-sky-300/20 bg-sky-400/10 text-xl font-semibold text-sky-200">
              {{ companyInitial }}
            </div>
            <div>
              <p class="text-sm font-semibold text-sky-100">{{ form.company_name || '未填写企业名称' }}</p>
      ...
      <form v-else class="space-y-6" @submit.prevent="saveBasicInfo">
        <section class="rounded-2xl border border-sky-400/10 bg-surface/65 p-5 md:p-6 backdrop-blur-xl space-y-4">
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <label class="space-y-1">
              ...
              <input
                ...
                type="text"
                maxlength="64"
                class="w-full rounded-lg border bg-slate-950/30 px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-sky-300/50"
                :class="fieldErrors.company_name ? 'border-rose-300/50' : 'border-sky-300/20'"
                placeholder="请输入企业全称"
              />
            ...
            <label class="space-y-1">
              ...
              <input
                ...
                maxlength="64"
                class="w-full rounded-lg border bg-slate-950/30 px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-sky-300/50"
                :class="fieldErrors.credit_code ? 'border-rose-300/50' : 'border-sky-300/20'"
            ...
            <label class="space-y-1 md:col-span-2">
              ...
              <textarea
                ...
                maxlength="512"
                class="w-full rounded-lg border bg-slate-950/30 px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-sky-300/50"
                :class="fieldErrors.registered_address ? 'border-rose-300/50' : 'border-sky-300/20'"
          ...
          <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap gap-3">
              <button
                type="submit"
                :disabled="saving"
                class="rounded-lg bg-sky-400 px-5 py-2.5 text-sm font-semibold text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {{ saving ? '保存中...' : '保存基本信息' }}
              </button>
            </div>
