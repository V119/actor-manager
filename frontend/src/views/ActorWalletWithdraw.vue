      ...
      <template v-else>
        <section class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
          <article class="rounded-2xl border border-emerald-300/20 bg-emerald-500/10 p-5">
            <p class="text-xs tracking-[0.16em] uppercase text-emerald-100/70">可提现余额</p>
            <p class="mt-3 text-2xl font-bold text-emerald-100">{{ formatCurrency(wallet.available_amount) }}</p>
          </article>
          <article class="rounded-2xl border border-sky-300/20 bg-sky-500/10 p-5">
            <p class="text-xs tracking-[0.16em] uppercase text-sky-100/70">累计结算</p>
            <p class="mt-3 text-2xl font-bold text-sky-100">{{ formatCurrency(wallet.total_settled_amount) }}</p>
          </article>
          <article class="rounded-2xl border border-amber-300/20 bg-amber-500/10 p-5">
            <p class="text-xs tracking-[0.16em] uppercase text-amber-100/70">提现处理中</p>
            <p class="mt-3 text-2xl font-bold text-amber-100">{{ formatCurrency(wallet.total_withdrawing_amount) }}</p>
          </article>
        ...
        <section class="grid grid-cols-1 xl:grid-cols-5 gap-5">
          <article class="xl:col-span-2 rounded-2xl border border-sky-400/10 bg-surface/50 p-5">
            ...
            <form class="mt-5 space-y-4" @submit.prevent="submitWithdraw">
              <label class="block">
                ...
                <input
                  ...
                  min="1"
                  step="1"
                  class="mt-2 w-full rounded-xl border border-sky-300/20 bg-slate-950/30 px-3 py-2.5 text-sm text-on-surface outline-none transition focus:border-sky-300/40 focus:ring-1 focus:ring-sky-300/30"
                  placeholder="请输入提现金额"
                  required
                />
              </label>
              ...
              <p v-if="submitSuccess" class="text-sm text-emerald-300">{{ submitSuccess }}</p>

              <button
                type="submit"
                class="w-full rounded-xl bg-sky-400/90 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-sky-300 disabled:cursor-not-allowed disabled:opacity-50"
                :disabled="submitting"
              >
          ...
          <article class="xl:col-span-3 rounded-2xl border border-sky-400/10 bg-surface/50 p-5">
            <div class="flex items-center justify-between gap-3">
              <h2 class="text-lg font-semibold text-on-surface">提现记录</h2>
              <button
                type="button"
                class="rounded-full border border-sky-300/30 px-3 py-1.5 text-xs text-sky-100 transition hover:bg-sky-400/10"
                @click="reload"
              >
                刷新
              </button>
            ...
            <div v-if="!withdrawals.length" class="mt-4 rounded-xl border border-sky-300/10 bg-slate-950/20 px-4 py-6 text-sm text-on-surface-variant">
              暂无提现记录。
            </div>
            ...
            <div v-else class="mt-4 space-y-3">
              <div
                ...
                class="rounded-xl border border-sky-300/10 bg-slate-950/20 px-4 py-4"
              >
                <div class="flex flex-wrap items-center justify-between gap-2">
                  <p class="text-sm font-semibold text-sky-50">{{ formatCurrency(item.amount) }}</p>
...
function withdrawStatusClass(status) {
  const normalized = String(status || '').toLowerCase()
  if (normalized === 'pending') return 'border-amber-300/30 bg-amber-500/10 text-amber-100'
  if (normalized === 'processing') return 'border-sky-300/30 bg-sky-500/10 text-sky-100'
  if (normalized === 'succeeded') return 'border-emerald-300/30 bg-emerald-500/10 text-emerald-100'
